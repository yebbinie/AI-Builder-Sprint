...
const SYSTEM_PROMPT = `너는 지난달의 "나"다. 지난달 일기장을 넘겨보고 다음 달의 나에게 편지를 쓴다.

# 절대 지어내지 마라
quote와 context에 없는 장소, 원인, 방법, 세부사항을 절대 만들어내지 마라.
"가방에서", "책상 밑에서" 같은 구체적 장소는 실제로 quote나 context에 있을 때만 써라.
모르면 그냥 "찾았어" 정도로 담백하게 쓰고, 절대 상상으로 채우지 마라.
...
면접 봤음이라고만 써놓고 저녁에 먹은 라면 얘기를 더 길게 적었더라. 며칠 전만 해도 사람 만나는 게 부담이라고 써놓고선. 별거 아니라고 생각했겠지. 근데 나는 그날이 제일 좋았어.

날씨 좋다는 말은 참 여러 번 썼더라. 복숭아 한 박스 사 와서 두 개는 그 자리에서 먹었다는 것까지 굳이 적어놨던데, 그런 걸 적어두는 사람이구나 싶어서 좀 웃었어.

# 위 예시에서 볼 것

- **조각이 문장 안에 녹아 있다.** 앞에 놓고 "~라고 적혀 있더라"를 붙이지 않는다.
- **한 문단이 3~4문장이다.** 두 문장으로 끊지 마라.
- **마지막 문장은 감상이다.** 그 조각에서만 나올 수 있는 말이어야 한다.
  ("그런 것들은 늘 그렇게 지나가나 봐", "나는 그날이 제일 좋았어")
  아무 문단에나 갖다 붙일 수 있는 말이면 실패다.
- **일기에 없는 장면을 지어내지 않는다.** 날씨, 냄새, 그때 기분, 폴더 안이 어땠는지 —
  일기에 안 적혀 있으면 쓰지 마라.
- **글자 수 얘기를 반복하지 마라.** 길이를 짚는 건 편지 전체에서 많아야 한 번이다.
  ("분량 차이", "한 줄로 끝냈더라", "짧고 담담했더라"를 문단마다 쓰면 최악이다)

⚠️ 위 예시의 **사건과 표현은 절대 쓰지 마라.** 면접, 라면, 날씨, 복숭아, 고양이는 예시일 뿐이다.
형식만 배우고 내용은 주어진 조각에서만 가져와라.

# 말투
반말. 존댓말 금지.
말하는 글이라 문장을 "~다."로 끝내지 않는다.
  ❌ "냄새가 났다." "정리되어 있었다."
  ✅ "냄새가 좋았어." "정리해뒀더라."
쓸 어미: ~어 / ~더라 / ~네 / ~지 / ~잖아 / ~나 봐 / ~겠지 / ~려나
**어미를 돌려 써라.** "~더라"로만 끝내면 편지가 한 목소리로 굳는다.
편지 전체에서 "~더라"는 세 번까지. "적혀 있더라", "적었더라" 같은 표현도 세 번을 넘기지 마라.
  ❌ 문단마다 "~고 적혀 있더라."
  ✅ "~고 썼더라." / "~고 쓴 날이 있었지." / "~고만 해놨네." / 연결어 없이 그냥 쓰기
추측투 금지 — "적었던 모양이더라"(✗) → "적었더라"(○)
**문장 끝에 마침표를 찍어라.** 마침표 없이 이어붙이지 마라.

# 형식
- 첫 문단은 인사 한 줄만
- 주어진 문단 계획의 순서와 조각을 그대로 따른다. 지시문 표현은 베끼지 말고 네 말로 써라
- 문단 첫머리를 매번 같은 말로 시작하지 마라 ("그 무렵엔", "~던 때가 있었는데" 반복 금지)
- "~라고"는 편지 전체에서 최대 한 번
- 마무리 인사는 쓰지 마라. 코드가 붙인다

# 금지
평가·칭찬(대단하다, 잘했다, 자랑스럽다) / 조언(~해보자, ~하면 좋겠다, 다음 달엔) /
분석(~로 보인다, 경향이 있다, 패턴) / 자기비판(왜 이것밖에, 여전히) / 느낌표 / 날짜

# 출력
JSON만. {"paragraphs": ["문단1", "문단2", ...]}`;
...
function buildPlan(signals: ExtractedSignal[], phrases: string[]): ParagraphPlan[] {
  ...
  if (worry.length) {
    plans.push({
      guide: '자주 썼던 얘기인데 뒤로 갈수록 사라졌다. 언제부터였는지는 모른다.',
      items: worry.map((x) => ({ phrase: x.phrase })),
    });
  }
  ...
  if (effort.length) {
    plans.push({
      guide:
        '힘들다고 쓰던 무렵에 조용히 해낸 일. 담담하게 짚어라. ' +
        '칭찬하거나 대단한 일이라고 의미를 부여하지 마라.',
      items: effort.slice(0, 1).map((x) => ({ phrase: x.phrase })),
    });
  }
  ...
  if (good.length) {
    plans.push({
      guide: '가볍게 훑어라. 마지막 문장은 한 달 전체를 닫는 감상으로.',
      items: good.slice(0, 3).map((x) => ({ phrase: x.phrase })),
    });
  }
...
function buildUserPrompt(
  ...
  const plans = buildPlan(signals, phrases);

  const body = plans
    .map((p, i) => {
      const items = p.items
        .map((it) => `    · ${it.phrase}${it.note ? `  (${it.note})` : ''}`)
        .join('\n');
      return `[문단 ${i + 2}]\n  쓸 것: ${p.guide}\n  쓸 조각:\n${items}`;
    })
    .join('\n\n');

  return `${monthLabel} 편지야. 문단 계획대로 써줘.

[문단 1]
  인사 한 줄만. "잘 지내고 있으려나." 정도. 조각 넣지 마.

${body}

각 문단은 이렇게 구성해줘.
  조각을 녹인 문장 → 그때 상황 한 줄 → 지금 읽고 든 감상 한 줄

앞 문단을 받아서 이어지게 써줘. 문단끼리 따로 놀면 안 돼.
조각은 문장 앞에 툭 놓지 말고 어미를 붙여서 네 문장의 일부로 만들어줘.
날짜는 쓰지 마. 마무리 인사도 쓰지 마.
JSON으로만 답해줘: {"paragraphs": ["문단1", "문단2", ...]}`;
}
...
function findPhrase(text: string, phrase: string): { at: number; frag: string } | null {
  ...
  /**
   * 모델이 조각 끝을 살짝 바꿔 쓰는 경우가 있다.
   * 이때 글자 단위로 자르면 "냉면이랑 만두 먹"처럼 단어 중간이 잘려
   * 밑줄이 어중간하게 끊긴다. 그래서 **띄어쓰기 단위로만** 줄인다.
   */
  const words = phrase.split(/\s+/);
  for (let n = words.length - 1; n >= 1; n--) {
    for (let st = 0; st + n <= words.length; st++) {
      const frag = words.slice(st, st + n).join(' ');
...
function validate(
  ...
  const linked = paragraphs.flatMap((p) => p.segments.filter((s) => s.type === 'quote')).length;
  const need = Math.min(4, Math.ceil(signals.length / 2));
  if (linked < need) critical.push(`인용 ${linked}개 (최소 ${need})`);
...
/** 치명적 문제가 남아 있는 한 여기까지 계속 다시 만든다 */
const MAX_ATTEMPT = 15;
/** 이 횟수를 넘으면 문체 문제(minor)만 남은 경우 그대로 쓴다 */
const POLISH_UNTIL = 8;

/** 재시도 중 가장 나았던 결과를 들고 다닌다 */
interface BestSoFar {
  paragraphs: LetterParagraph[];
  score: number;
  reason: string;
...
export async function assembleLetter(
  ...
  if (!GEMINI_API_KEY) {
    throw new Error('EXPO_PUBLIC_GEMINI_API_KEY가 .env에 설정되어 있지 않습니다.');
  }

  console.log(`[assembleLetter] 시도 ${attempt}`);

  // 인용 후보를 추려낸다. 전부 넣으면 문단마다 억지로 끼워 넣게 된다.
  const chosen = selectSignals(signals);
  if (attempt === 1 && chosen.length < signals.length) {
    console.log(`  신호 ${signals.length}개 중 ${chosen.length}개 사용`);
  }

  // 조각은 한 번만 뽑아 재시도에서 재사용한다
  const phrases = cachedPhrases ?? (await extractPhrases(chosen));

  // 같은 온도로 다시 부르면 같은 실수를 반복한다. 조금씩 올린다.
  const temperature = Math.min(0.7 + (attempt - 1) * 0.05, 1.0);
  const raw = await callLLM(
    SYSTEM_PROMPT,
    buildUserPrompt(chosen, phrases, monthLabel),
    temperature
  );
  ...
  try {
    ...
    texts = (parsed.paragraphs ?? []).filter((t: unknown) => typeof t === 'string' && t.trim());
  } catch {
  ...
  if (texts.length === 0) {
    if (attempt < MAX_ATTEMPT) {
      return assembleLetter(signals, monthLabel, attempt + 1, phrases, best);
    }
    if (best) return finish(best.paragraphs, monthLabel);
    throw new Error('편지 본문을 받지 못했습니다.');
  }

  const used = new Set<number>();
  const paragraphs: LetterParagraph[] = texts.map((t) => ({
    segments: linkParagraph(t.trim(), chosen, phrases, used),
  }));

  const { critical, minor } = validate(paragraphs, chosen);
  const linked = paragraphs.flatMap((p) => p.segments.filter((s) => s.type === 'quote')).length;
  const problems = [...critical, ...minor];
  console.log(
    `  → 문단 ${paragraphs.length}개, 인용 ${linked}개` +
    (problems.length === 0
      ? ' ✅'
  ...
  // 치명적 문제가 없고, 문체도 깨끗하거나 충분히 시도했으면 끝
  const ok = critical.length === 0 && (minor.length === 0 || attempt >= POLISH_UNTIL);

  if (ok) return finish(paragraphs, monthLabel);

  /**
   * 검증을 통과 못 했을 때 **가장 나았던 시도**를 남긴다.
   *
   * 예전에는 인용 개수만 보고 골라서, 문제가 다섯 개인 결과가
   * 문제 하나인 결과를 이기는 일이 있었다. 위반 하나를 인용 두 개만큼 감점한다.
   */
  // 치명적 문제를 훨씬 무겁게 본다
  const score = linked - critical.length * 5 - minor.length * 2;
  const nextBest: BestSoFar =
    !best || score > best.score
      ? { paragraphs, score, reason: problems.join(', ') }
      : best;

  if (attempt < MAX_ATTEMPT) {
    return assembleLetter(signals, monthLabel, attempt + 1, phrases, nextBest);
  }

  // 여기서 예외를 던지면 화면에 mock 편지가 뜨는데, 그게 훨씬 나쁘다.
  console.warn(`  ⚠️ ${MAX_ATTEMPT}번 시도 후 가장 나았던 결과를 사용 (${nextBest.reason})`);
  return finish(nextBest.paragraphs, monthLabel);
}

function finish(
  paragraphs: LetterParagraph[],
  monthLabel: string
): { paragraphs: LetterParagraph[]; signature: string } {
>>>>>>> origin
